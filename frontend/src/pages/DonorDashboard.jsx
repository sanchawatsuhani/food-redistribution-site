import { useState, useEffect, useCallback } from 'react';
import { useNavigate } from 'react-router-dom';
import { useAuth } from '../context/AuthContext';
import { getSocket } from '../services/socket';
import api from '../services/api';
import AddFoodForm from '../components/AddFoodForm';
import FoodCard from '../components/FoodCard';
import StatusBadge from '../components/StatusBadge';
import { FiPackage, FiCheckCircle, FiClock, FiTrendingUp } from 'react-icons/fi';

export default function DonorDashboard() {
  const { user, loading: authLoading } = useAuth();
  const navigate = useNavigate();
  const [listings, setListings] = useState([]);
  const [pickups, setPickups] = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (authLoading) return;
    if (!user || user.role !== 'donor') navigate('/login');
  }, [user, authLoading, navigate]);

  const fetchData = useCallback(async () => {
    try {
      const [foodRes, pickupRes] = await Promise.all([
        api.get('/food/mine'), api.get('/pickups')
      ]);
      setListings(foodRes.data.listings);
      setPickups(pickupRes.data.pickups);
    } catch (err) { console.error(err); }
    finally { setLoading(false); }
  }, []);

  useEffect(() => {
    if (authLoading || !user || user.role !== 'donor') return;
    fetchData();
  }, [authLoading, user, fetchData]);

  // Real-time Socket.IO listeners
  useEffect(() => {
    if (authLoading || !user || user.role !== 'donor') return;
    const socket = getSocket();
    const onClaimed = (data) => {
      setListings(prev => prev.map(l =>
        l.id === data.listing.id ? data.listing : l
      ));
      fetchData(); // refresh pickups too
    };
    const onExpired = (data) => {
      setListings(prev => prev.map(l =>
        l.id === data.foodId ? { ...l, status: 'Expired' } : l
      ));
    };
    socket.on('food:claimed', onClaimed);
    socket.on('food:expired', onExpired);
    socket.on('pickup:completed', fetchData);
    return () => {
      socket.off('food:claimed', onClaimed);
      socket.off('food:expired', onExpired);
      socket.off('pickup:completed', fetchData);
    };
  }, [authLoading, user, fetchData]);

  if (authLoading) {
    return (
      <div className="page">
        <div className="loading" style={{ minHeight: '40vh' }}><div className="spinner"></div></div>
      </div>
    );
  }
  if (!user || user.role !== 'donor') return null;

  const available = listings.filter(l => l.status === 'Available').length;
  const claimed = listings.filter(l => l.status === 'Claimed').length;
  const expired = listings.filter(l => l.status === 'Expired').length;

  const handleCompletePickup = async (pickupId) => {
    try {
      await api.put(`/pickups/${pickupId}/complete`);
      fetchData();
    } catch (err) { console.error(err); }
  };

  return (
    <div className="page">
      <div className="dashboard">
        <div className="dashboard-header">
          <h1>Welcome, {user.displayName}</h1>
          <p>Manage your food donations and track your impact</p>
        </div>

        {/* Stats */}
        <div className="dashboard-stats">
          <div className="dash-stat-card">
            <div className="dash-stat-icon primary"><FiPackage size={22} /></div>
            <div><div className="dash-stat-value">{listings.length}</div><div className="dash-stat-label">Total Listings</div></div>
          </div>
          <div className="dash-stat-card">
            <div className="dash-stat-icon accent"><FiCheckCircle size={22} /></div>
            <div><div className="dash-stat-value">{claimed}</div><div className="dash-stat-label">Claimed</div></div>
          </div>
          <div className="dash-stat-card">
            <div className="dash-stat-icon warning"><FiClock size={22} /></div>
            <div><div className="dash-stat-value">{available}</div><div className="dash-stat-label">Available</div></div>
          </div>
          <div className="dash-stat-card">
            <div className="dash-stat-icon danger"><FiTrendingUp size={22} /></div>
            <div><div className="dash-stat-value">{expired}</div><div className="dash-stat-label">Expired</div></div>
          </div>
        </div>

        {/* Add Food Form */}
        <div className="add-food-section">
          <AddFoodForm onSuccess={fetchData} />
        </div>

        {/* Pickups */}
        {pickups.length > 0 && (
          <div className="dashboard-section">
            <div className="section-header">
              <h2>Active Pickups</h2>
            </div>
            <div className="pickups-list">
              {pickups.filter(p => p.status === 'Pending').map(p => (
                <div className="pickup-card" key={p.id}>
                  <div className="pickup-info">
                    <div className="pickup-food">{p.foodItem}</div>
                    <div className="pickup-details">
                      <span>NGO: {p.ngoName}</span>
                      <span>Area: {p.area}</span>
                    </div>
                  </div>
                  <StatusBadge status={p.status} />
                  <div className="pickup-actions">
                    <button className="btn btn-accent btn-sm" onClick={() => handleCompletePickup(p.id)}>
                      Complete
                    </button>
                  </div>
                </div>
              ))}
            </div>
          </div>
        )}

        {/* Listings */}
        <div className="dashboard-section">
          <div className="section-header">
            <h2>Your Food Listings <span className="live-indicator"><span className="live-dot"></span> Live</span></h2>
          </div>
          {loading ? (
            <div className="loading"><div className="spinner"></div></div>
          ) : listings.length === 0 ? (
            <div className="empty-state">
              <div className="empty-state-icon"><FiPackage size={48} /></div>
              <div className="empty-state-title">No listings yet</div>
              <div className="empty-state-text">Create your first food listing to start making a difference!</div>
            </div>
          ) : (
            <div className="food-grid">
              {listings.map(l => <FoodCard key={l.id} listing={l} />)}
            </div>
          )}
        </div>
      </div>
    </div>
  );
}
